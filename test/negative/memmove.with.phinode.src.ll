...
%"group" = type { [1 x i64], [1 x i64], [1 x i64], [1 x i64] }
%"accessor" = type { %"AccessorImplDevice", i32 addrspace(1)* }
%"AccessorImplDevice" = type { [1 x i64], [1 x i64], [1 x i64] }
%class.kfunc = type <{ %"accessor", %"group", i32, [4 x i8] }>
...
define weak_odr dso_local spir_kernel void @test_func_object() local_unnamed_addr comdat !kernel_arg_buffer_location !4 {
entry:
  %0 = alloca %"accessor", align 8
  %1 = addrspacecast %"accessor"* %0 to %"accessor" addrspace(4)*
...
entry.merge_crit_edge:                            ; preds = %entry
  %3 = bitcast %"accessor" addrspace(4)* %1 to i8 addrspace(4)*
  br label %merge

leader:                                           ; preds = %entry
  %4 = bitcast %"accessor" addrspace(4)* %1 to i8 addrspace(4)*
  br label %merge
