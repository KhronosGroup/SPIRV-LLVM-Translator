...
; CHECK-SPIRV: LoopMerge [[MERGEBLOCK:[0-9]+]] [[CONTINUE:[0-9]+]] 2
...
; CHECK-SPIRV: LoopMerge [[MERGEBLOCK:[0-9]+]] [[CONTINUE:[0-9]+]] 256 8
; CHECK-SPIRV: BranchConditional {{.*}} {{.*}} [[MERGEBLOCK]]
...
; CHECK-SPIRV: Label [[HEADER:[0-9]+]]
  ...
  %tobool = icmp ne i32 %rem, 0
; Per SPIRV spec p3.23 "Unroll" loop control = 0x1
; CHECK-SPIRV: LoopMerge [[MERGEBLOCK:[0-9]+]] [[CONTINUE:[0-9]+]] 1
...
; CHECK-SPIRV: Function
; CHECK-SPIRV: Label
; CHECK-SPIRV: Branch [[Header:[0-9]+]]
; CHECK-SPIRV: Label [[Return:[0-9]+]]
...
define spir_func void @unroll_full() {
  ...
  ; CHECK-LLVM: br i1 %{{.*}}, !llvm.loop ![[FULL:[0-9]+]]
...
; CHECK-LLVM: ![[UNROLLDISABLE]] = distinct !{![[UNROLLDISABLE]], ![[DISABLE:[0-9]+]]}
; CHECK-LLVM: ![[DISABLE]] = !{!"llvm.loop.unroll.disable"}
; CHECK-LLVM: ![[UNROLLCOUNT]] = distinct !{![[UNROLLCOUNT]], ![[COUNT:[0-9]+]]}
; CHECK-LLVM: ![[COUNT]] = !{!"llvm.loop.unroll.count", i32 8}
; CHECK-LLVM: ![[UNROLLENABLE1]] = distinct !{![[UNROLLENABLE1]], ![[ENABLE:[0-9]+]]}
; CHECK-LLVM: ![[ENABLE]] = !{!"llvm.loop.unroll.enable"}
; CHECK-LLVM: ![[UNROLLENABLE2]] = distinct !{![[UNROLLENABLE2]], ![[ENABLE]]}
; CHECK-LLVM: ![[FULL]] = distinct !{![[FULL]], ![[UNROLLFULL:[0-9]+]]}
