...
- may appear at module scope, in the locations permitted for
  ...
  *OpVariable*), the instruction must appear after all type, constant,
  ...
  instruction is non-semantic and its result is never consumed. Because
  an *OpExtInst* may not forward-reference its operands, such a record
  must be emitted as *OpExtInstWithForwardRefsKHR*, and the module must
  declare the *SPV_KHR_relaxed_extended_instruction* extension.
...
so the record must be encoded as *OpExtInstWithForwardRefsKHR* and the
module must declare the *SPV_KHR_relaxed_extended_instruction*
extension. +
 +
'Result Type' must be *OpTypeVoid*. +
 +
...
NOTE: The opcode is *OpExtInst* (12) only when the 'Target' precedes
this record; when 'Target' is a forward reference (the recommended
module-scope placement) the opcode is *OpExtInstWithForwardRefsKHR*
(4433) and *SPV_KHR_relaxed_extended_instruction* must be declared.
